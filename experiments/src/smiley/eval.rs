...
impl<'a> Value<'a> {
  ...
  fn to_body(&self) -> Result<&'a Expr<Smiley>, TypeError> {
    match self {
      ...
      _ => Err(TypeError { expected: "function".to_string() }),
    }
  }
...
impl Shape {
  fn translate(self, x_offset: f64, y_offset: f64) -> Self {
    ...
    match self {
      Self::Line { start, end } => {
        Self::Line { start: start + offset, end: end + offset }
      }
      Self::Ellipse { transform, center } => {
        Self::Ellipse { transform, center: center + offset }
      }
    }
  }
  ...
  fn linear_transform(self, matrix: Matrix2<f64>) -> Self {
    match self {
      Self::Line { start, end } => {
        Self::Line { start: matrix * start, end: matrix * end }
      }
      Self::Ellipse { transform, center } => {
        Self::Ellipse { transform: matrix * transform, center }
      }
    }
  }

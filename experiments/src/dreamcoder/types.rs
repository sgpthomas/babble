...
impl<'a> From<RawType<'a>> for Type {
  fn from(raw_type: RawType<'a>) -> Self {
    let RawType { constructor, mut arguments } = raw_type;
...
impl From<Type> for RawType<'_> {
  fn from(typ: Type) -> Self {
    match typ {
      Type::Simple { name } => {
        Self { constructor: name.as_str(), arguments: Vec::new() }
      }
      Type::Compound { constructor, arguments } => {
...
impl Display for Type {
  fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
    match self {
      ...
      Type::Compound { constructor, arguments } => util::parens(1, f, |f| {
        write!(f, "{constructor}")?;
...
mod tests {
  ...
  fn type_display() {
    ...
    let expected =
      "(int -> list int) -> list (list bool) -> result int (list string)";

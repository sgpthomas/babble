...
impl<'a> Context<'a> {
  fn new() -> Self {
    Self { libs: HashMap::new(), args: Vec::new() }
  }
  ...
  fn eval(&self, expr: &'a Expr<Drawing>) -> Result<Value<'a>, TypeError> {
    match (expr.0.operation(), expr.0.args()) {
      ...
      (Drawing::Square, []) => Ok(Value::Shapes(vec![
        Shape::Line { start: (-0.5, -0.5), end: (0.5, -0.5) },
        Shape::Line { start: (0.5, -0.5), end: (0.5, 0.5) },
        Shape::Line { start: (0.5, 0.5), end: (-0.5, 0.5) },
        Shape::Line { start: (-0.5, 0.5), end: (-0.5, -0.5) },
      ])),
      (Drawing::Rect, [w, h]) => match (self.eval(w)?, self.eval(h)?) {
        (Value::Num(w), Value::Num(h)) => Ok(Value::Shapes(vec![
          Shape::Line { start: (-w / 2.0, -h / 2.0), end: (w / 2.0, -h / 2.0) },
          Shape::Line { start: (w / 2.0, -h / 2.0), end: (w / 2.0, h / 2.0) },
          Shape::Line { start: (w / 2.0, h / 2.0), end: (-w / 2.0, h / 2.0) },
          Shape::Line { start: (-w / 2.0, h / 2.0), end: (-w / 2.0, -h / 2.0) },
        ])),
        _ => Err(TypeError { expected: "numbers".to_string() }),
      },
      ...
      (Drawing::Apply, [fun, arg]) => {
        if let Value::Lambda(body, mut args) = self.eval(fun)? {
          ...
          Err(TypeError { expected: "function".to_string() })
        }
      }
      ...
      (Drawing::Repeat, [expr, times, mat]) => {
        ...
        let t = match self.eval(times) {
          ...
          _ => {
            panic!("Second argument to Repeat must be a non-negative number")
          }
        };
        ...
        for _i in 0..t {
          ...
          prev = prev
            .map_shapes(|shape| shape.translate(tx, ty).rotate(rot).scale(sc));
        }
      ...
      (Drawing::List, exprs) => {
        ...
        for (i, expr) in exprs.iter().enumerate() {
          ...
          let new_box =
            Shape::shapes_bounding_box(&value.clone().into_shapes()?);
...
fn f64_to_usize(f: f64) -> usize {
  ...
  assert!(f <= usize::MAX as f64, "number is too big to convert to an integer");
  f.round() as usize
}
...
impl Value<'_> {
  fn to_float(&self) -> Result<f64, TypeError> {
    match *self {
      Self::Num(f) => Ok(f),
      _ => Err(TypeError { expected: "number".to_string() }),
    }
  }
  ...
  fn into_shapes(self) -> Result<Vec<Shape>, TypeError> {
    match self {
      ...
      _ => Err(TypeError { expected: "shapes".to_string() }),
    }
  }
...
impl Shape {
  ...
  {
    match self {
      Self::Circle { center: (x, y), radius } => {
        let (x1, y1) = transform_point(x, y);
        let (x2, y2) = transform_point(x + radius, y);
        let new_radius = (x2 - x1).hypot(y2 - y1);
        Self::Circle { center: (x1, y1), radius: new_radius }
      }
      Self::Line { start: (x1, y1), end: (x2, y2) } => Self::Line {
  ...
  fn bounding_box(&self) -> BoundingBox {
    match self {
      Self::Circle { center: (x, y), radius } => BoundingBox {
        lower_left: (x - radius, y - radius),
        upper_right: (x + radius, y + radius),
      },
      Self::Line { start: (x1, y1), end: (x2, y2) } => BoundingBox {
  ...
  fn shapes_bounding_box(shapes: &[Shape]) -> BoundingBox {
    shapes
      .iter()
      .fold(BoundingBox::EMPTY, |bbox, shape| bbox.union(&shape.bounding_box()))
  }
}

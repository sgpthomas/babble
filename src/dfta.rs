  ...
  S: Ord,
{
  ...
  pub(crate) fn new() -> Self {
    Self { by_operation: BTreeMap::new(), by_output: BTreeMap::new() }
  }
  ...
  S: Clone + Ord,
{
    ...
    I: IntoIterator<Item = S>,
  {
    ...
    self.by_output.entry(output).or_default().insert((operation, inputs));
  }
        ...
        for (op2, rules2) in &self.by_operation {
          for (inputs2, output2) in rules2 {
            ...
            if op1 == op2 {
              ...
              let new_inputs =
                inputs1.iter().cloned().zip(inputs2.iter().cloned());
              new_dfta.add_rule(
                (op1.clone(), op2.clone()),
                new_inputs,
                new_output,
  ...
  S: Ord,
{
  ...
  /// return `None`.
  #[must_use]
  pub(crate) fn get_by_output(
    &self,
    output: &S,
  ) -> Option<&BTreeSet<(Op, Vec<S>)>> {
